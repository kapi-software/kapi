/**
 * @file locales.test.ts
 * @description 语言包结构一致性测试：zh-CN 与 en-US 的 key 集合必须完全一致
...
function flattenKeys(obj: Record<string, unknown>, prefix = ''): string[] {
  return Object.entries(obj).flatMap(([key, value]) => {
    ...
    // 嵌套对象继续下钻 / Drill into nested objects
...
describe('语言包结构一致性 / Locale parity', () => {
  ...
  it('所有文案为非空字符串 / All values are non-empty strings', () => {
    for (const keys of [flattenKeys(zhCN), flattenKeys(enUS)]) {
      // flattenKeys 只返回叶子路径，此处仅验证数量非零
      // flattenKeys returns leaf paths only; verify non-empty here
